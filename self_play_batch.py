...
def run_parallel_self_play_batch(
    ...
    successful_games = 0
    for i, (samples, result_str, num_moves, pgn_str) in enumerate(results):
        indexed_results.append((i, samples, result_str, num_moves, pgn_str)) # Store index
        if samples is not None and result_str is not None and pgn_str is not None:
            ...
            # Don't store pgn_str separately here, we use indexed_results

            if samples or num_moves > 0:
                 successful_games += 1
        else:
            print(f"Warning: Worker for game index {i} returned invalid data.")
    ...
    if num_pgns_to_save > 0:
        ...
        saved_indices = set() # Keep track of which game indices we've already saved
        pgns_saved_count = 0

        print(f"Saving PGNs (first {num_pgns_to_save} + all decisive) to: {pgn_dir}")

        # 1. Save the first 'num_pgns_to_save' games
        for i in range(min(num_pgns_to_save, len(indexed_results))):
            original_index, _, result_str, _, pgn_str_to_save = indexed_results[i]
            if pgn_str_to_save:
                pgn_filename = os.path.join(pgn_dir, f"game_{original_index:04d}_{result_str.replace('/','-')}.pgn") # Add result to filename
                try:
                    with open(pgn_filename, "w", encoding='utf-8') as f:
                        f.write(pgn_str_to_save)
                    saved_indices.add(original_index)
                    pgns_saved_count += 1
                except Exception as e:
                    print(f"Error writing PGN file {pgn_filename}: {e}")
            else:
                print(f"Warning: Skipping empty PGN for initial game index {original_index}.")

        # 2. Save any additional games that had a decisive result (Win/Loss)
        for original_index, _, result_str, _, pgn_str_to_save in indexed_results:
            # Check if decisive AND not already saved
            if result_str in ["1-0", "0-1"] and original_index not in saved_indices:
                if pgn_str_to_save:
                    pgn_filename = os.path.join(pgn_dir, f"game_{original_index:04d}_{result_str.replace('/','-')}.pgn") # Add result
                    try:
                        with open(pgn_filename, "w", encoding='utf-8') as f:
                            f.write(pgn_str_to_save)
                        saved_indices.add(original_index) # Mark as saved
                        pgns_saved_count += 1
                    except Exception as e:
                        print(f"Error writing PGN file {pgn_filename}: {e}")
                else:
                    print(f"Warning: Skipping empty PGN for decisive game index {original_index}.")
